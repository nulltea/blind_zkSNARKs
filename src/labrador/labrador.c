...
void init_proof(proof *pi, const witness *wt, int quadratic, int tail) {
  ...
  if(cpp->kappa > 32)
    printf("ERROR: Inner commitments not secure!\n");
  if(cpp->kappa1 > 32)
    printf("ERROR: Outer commitments not secure!\n");
...
void init_constraint_raw(constraint *cnst, size_t r, size_t n, size_t deg, int quadratic) {
  ...
  buf = _aligned_alloc(64,r*n*sizeof(polx));
  cnst->phi = (polx*)buf;
  ...
  if(quadratic) {
    ...
    buf = _aligned_alloc(64,len*sizeof(polx));
...
void project(statement *ost, proof *pi, uint8_t jlmat[][ost->n][256*N/8], const witness *iwt) {
  ...
  if(normsq > hardbound*hardbound)
    printf("ERROR: Total witness norm too big for JL Projection\n");
  normsq *= 256;
  ...
  } while(jlproj_normsq(pi->p) > normsq);
...
{
  __attribute__((aligned(16)))
  uint8_t hashbuf[16+768+16];  // additional 16 bytes because of vector loads
  int64_t x;

  shake128(hashbuf,sizeof(hashbuf),h,16);
  memcpy(h,hashbuf,16);
  polxvec_jlproj_collapsmat24(cnst->phi,**jlmat,r*n,&hashbuf[16]);
  jlproj_collapsproj24(&x,p,&hashbuf[16]);
