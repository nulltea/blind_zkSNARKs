...
printv(f"auto-generated by lnp-quad-codegen.sage from {params_file}.")
printv(f"")

# check completeness conditions ([1], theorem 5.2.2)
...
# check simulatability conditions ([1], theorem 5.2.3)
if not (kmlwe >= 0 and kmlwe == m2 - kmsis - l - 1):
    err("protocol not simulatable")
printv(
    f"protocol is simulatable under MLWE({kmlwe},{kmsis+l+1},[-{nu},{nu}])")  # XXX extended MLWE - yes because stdev 2 is bimodal?

# check knowledge-soundness conditions ([1], theorem 5.2.4)

eknow = mpf(2)/mpf(Csize)
printv(
...
static const abdlop_params_t {name} = {{{{ {name}_ring, {name}_dcomp, {m1}, {m2}, {l}, {lext}, {kmsis}, {name}_Bsq, {nu}, {omega}, {omega_bits}, {eta}, {rejs1}, {log2stdev1}, {name}_scM1, {name}_stdev1sq, {rejs2}, {log2stdev2}, {name}_scM2, {name}_stdev2sq}}}};
"""
