...
static int test_twolayer() {
  ...
  if(ret) {
    printf("ERROR: verification of linear failed: %d\n",ret);
    goto end;
  }
  ...
  ret = verify(&st1,&wt1);
  if(ret) {
    printf("ERROR: verification of Chihuahua failed: %d\n",ret);
    goto end;
  }
  ...
  ret = verify(&st1,&wt1);
  if(ret) {
    printf("ERROR: verification of principle_reduce failed: %d\n",ret);
    goto end;
  }
...
static int test_pack() {
  ...
  if(ret)
    printf("ERROR: verify_composite_principle failed: %d\n",ret);
