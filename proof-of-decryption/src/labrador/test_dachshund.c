...
static int test_twolayer() {
  ...
  if(ret) {
    printf("ERROR: verification of prepare_linear failed: %d\n",ret);
    goto end;
  }
  ...
  ret = verify(&st1,&wt1);
  if(ret) {
    printf("ERROR: verification of Dachshund failed: %d\n",ret);
    goto end;
  }
  ...
  ret = verify(&st1,&wt1);
  if(ret) {
    printf("ERROR: verification of simple_reduce failed: %d\n",ret);
    goto end;
  }
  ...
  ret = verify(&st2,&wt2);
  if(ret) {
    printf("ERROR: verification of Labrador failed: %d\n",ret);
    goto end;
  }
  ...
  free_statement(&st1);
  if(ret) {
    printf("ERROR: reduce failed: %d\n",ret);
    goto end;
  }
  ret = verify(&st2,&wt2);
  if(ret) {
    printf("ERROR: verification of reduce failed: %d\n",ret);
    goto end;
  }
...
static int test_pack() {
  ...
  if(ret)
    printf("ERROR: verify_composite_simple failed: %d\n",ret);
