...
class polyring_t:
    ...
    def __init__(self, d, q: int):
        ...
        self.ptr = ffi.new("polyring_t",
                           [[self.mod_.ptr, d, coeffbits, log2d, moduliptr, nmoduli, self.inv2_.ptr]])
        self.val = self.ptr[0]
        self._locked = True
...
class poly_t:
    ...
    # switch to a smaller ring, internal function usd for multiplication of >64 degree rings
...
class polyvec_t:
    ...
    def set_elem(self,val,row,pos=None):
        ...
        """
        if pos==None: # setting a pol_t val to column col
            lib.polyvec_set_elem(self.ptr,row,val.ptr)
        else: #pos is an integer. setting an int or int_t val to column col, position pos
            if type(val) is int:
    ...
    def get_elem(self,row,pos=None):
        ...
        if pos==None: # getting a pol_t from column col
            polptr=lib.polyvec_get_elem(self.ptr,row)
            return poly_t(self.ring,polptr)
    ...
    def __mul__(self,a):
        ...
        if type(a) is poly_t:
            if self.ring.deg != 64:
                ...
                res=self.copy_zero()
                lib.polyvec_scale2(res.ptr,a.ptr,self.ptr)
        if type(a) is polyvec_t: # do inner product
            if self.ring.deg != 64:
                assert self.ring.deg==a.ring.deg and self.ring.mod==a.ring.mod
                assert self.dim==a.dim
...
class polymat_t:
    ...
    def get_row(self,row):
        ...
        return polyvec_t(self.ring,self.cols,resptr)
    ...
    def get_col(self,col):
        ...
        return polyvec_t(self.ring,self.rows,resptr)
...
class lin_prover_state_t:
    ...
    def prove(self, coins: bytes = None):
        ...
        proof = ffi.new(f"char[{int(math.ceil(self.expected_prooflen * 1.2))}]")
